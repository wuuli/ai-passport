(() => {
  ...
  const descriptions = {
    home: ['不比手快，比时间感', '两名特工共用一台终端，轮流估算同一个目标时长。误差更小者赢一轮，先赢三轮者获胜。首页 UP 切模式，DOWN 切声音。'],
    target: ['记住目标，按 OK 开始', '目标在 1.0–3.0 秒之间随机抽取，间隔 0.5 秒。不是越快越好，而是越接近目标越好。'],
    ...
    ai: ['教官独立挑战', 'AI 不参考你的成绩，按目标时长独立生成估时。约 1.1 秒后完成，同样先封存，等待你确认揭晓。'],
  ...
  const scenarios = [
    ...
    { id: 'timeout', name: '超时自动停止', copy: '目标 2 秒，5 秒时自动停止。先手超时后仍先交接，不提前公开成绩。', states: ['timeout', 'timeout-sealed', 'timeout-result'] },
  ];
  let state = Duel.create(), soundOn = true, frozen = false, demo = false, menu = false;
  let timer, holdTimer, heldKey = null, pendingAi = 2000, focused = true;
  let lastActivity = performance.now(), dimmed = false;
  let chosen = scenarios[0], scenarioStep = -1;
  const nameOf = player => player === 0 ? '特工 01' : state.mode === 'ai' ? 'AI 教官' : '特工 02';
  ...
  function screenMarkup(view) {
    ...
    if (menu) {
      ...
      case 'home':
        ...
        body = text('掐秒挑战', 0, 8, 224, 'fw-title fw-gold') + text('不比手快，比时间感', 34) + duo(60)
          + text(view.mode === 'duo' ? '双人对抗 · 五局三胜' : '单人练习 · AI 教官', 176)
      ...
      case 'handover':
        ...
        hint = 'OK 开始计时'; break;
      case 'ai':
        body = agent(1, false, 80, 35) + text('教官正在掐秒', 156) + text('你的成绩已锁定', 191);
        hint = '等待教官完成'; break;
      case 'sealed':
    ...
    return `<div class="fw-background"></div><div class="fw-header">${text(status, 5, 5, 180)}${text('--%', 7, 185, 50, 'fw-small')}</div><div class="fw-body">${body}</div><div class="fw-hint">${hint}</div>${text(menu ? '硬件菜单仅为示意' : '长按 OK 返回菜单', 300)}`;
  }
  ...
  function statusLabel(view, player) {
    ...
    return view.phase === 'timing' || view.phase === 'ai' ? '正在挑战' : view.phase === 'handover' ? '接过终端，直接开始' : '准备开始';
  }
  function render() {
    ...
    [0, 1].forEach(player => {
      ...
      seat.classList.toggle('active', !menu && ['target', 'timing', 'handover', 'ai'].includes(view.phase) && player === view.current);
    ...
    element('mode-duo').setAttribute('aria-pressed', view.mode === 'duo');
    element('mode-ai').setAttribute('aria-pressed', view.mode === 'ai');
    element('up').disabled = element('down').disabled = menu || view.phase !== 'home';
    ...
    element('game-round').textContent = menu ? '游戏外 · 菜单边界示意' : view.phase === 'home' ? '240 × 320 · 当前固件界面' : view.phase === 'final' ? '训练结束' : `第 ${view.round} 回合 · 五局三胜`;
    ...
    element('demo-label').textContent = frozen ? '画面预览 · 演示数据 · 已暂停' : demo ? '演示数据 · 按实机流程播放' : '自由试玩 · 当前固件流程';
  ...
  function orchestrate() {
    ...
    const deadlines = { timing: state.started + state.target + 3000, ai: state.phaseStarted + 1100, celebrate: state.phaseStarted + 1500, final: state.phaseStarted + 3500 };
    ...
    timer = setTimeout(() => {
      dispatch({ type: 'TICK', now: performance.now(), elapsed: pendingAi });
  ...
  function dispatch(action) {
    ...
    if (state.phase === 'ai' && before.phase !== 'ai') pendingAi = Duel.aiEstimate(state.target);
    let cue = ['OK', 'MODE'].includes(action.type) ? 'button' : undefined;
  ...
  function pressOk() {
    if (touch()) return;
    if (menu) { menu = false; reset(state.mode); return; }
    const now = performance.now();
    if (frozen) {
  ...
  function beginHold(key) {
    ...
    holdTimer = setTimeout(() => {
      releaseHold(); clearTimeout(timer); state = Duel.create(state.mode);
  ...
  function reset(mode = state.mode) {
    ...
    state = Duel.create(mode); menu = false; frozen = false; demo = false; dimmed = false; scenarioStep = -1;
  ...
  function preview(name) {
    ...
    state = Duel.create(state.mode);
    if (name !== 'home') state = Duel.prepare(state, 2000);
    ...
    if (['first', 'second', 'handover', 'timeout'].includes(name)) {
      if (name !== 'first') { state.elapsed[0] = name === 'timeout' ? 5000 : 1982; state.automatic[0] = name === 'timeout'; state.current = 1; }
      state.phase = state.mode === 'ai' && name !== 'first' ? 'ai' : ['handover', 'timeout'].includes(name) ? 'handover' : 'timing';
      state.started = 0;
    } else if (name === 'tie-target') { state.starter = state.current = state.mode === 'ai' ? 0 : 1; }
    else if (!['home', 'target'].includes(name)) {
      state.phase = 'sealed';
      state.elapsed = name.startsWith('tie-') ? [1980, 2020] : name.startsWith('timeout-') ? [5000, 2145] : [1982, 2145];
  ...
  document.addEventListener('keydown', event => {
    ...
    if (['ArrowUp', 'ArrowDown'].includes(event.code) && state.phase === 'home' && !menu) {
      event.preventDefault(); element(event.code === 'ArrowUp' ? 'up' : 'down').click();
    }
  });
  ...
  element('mode-duo').onclick = () => reset('duo');
  element('mode-ai').onclick = () => reset('ai');
  element('sound').onclick = toggleSound;
  element('up').onclick = () => { if (!touch()) dispatch({ type: 'MODE', now: performance.now() }); };
